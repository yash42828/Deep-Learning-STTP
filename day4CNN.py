...
weights = {
    'wc0': tf.get_variable('W0', shape=(3,3,1,16), initializer=tf.contrib.layers.xavier_initializer()), 
    'wc1': tf.get_variable('W1', shape=(3,3,16,32), initializer=tf.contrib.layers.xavier_initializer()), 
    ...
    'wd2': tf.get_variable('W5', shape=(128,64), initializer=tf.contrib.layers.xavier_initializer()), 
    'out': tf.get_variable('W6', shape=(64,n_classes), initializer=tf.contrib.layers.xavier_initializer()), 
}
biases = {
    'bc0': tf.get_variable('B0', shape=(16), initializer=tf.contrib.layers.xavier_initializer()),
    ...
    'bd2': tf.get_variable('B5', shape=(64), initializer=tf.contrib.layers.xavier_initializer()),
    'out': tf.get_variable('B6', shape=(10), initializer=tf.contrib.layers.xavier_initializer()),
}
...
with tf.Session() as sess:
    ...
    for i in range(training_iters):
        ...
        itrs = int(len(train_X)/batch_size)
        print('Batch Iterations: ' + str(itrs))
        for batch in range(itrs):
            ...
            loss, acc = sess.run([cost, accuracy], feed_dict={x: batch_x,
                                                              y: batch_y})
            print('Accuracy in Batch Iteration ' + str(batch) + ' : ' + str(acc))
